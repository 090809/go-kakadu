...
var ProjectWhere = struct {
	...
	ID:        whereHelperuint{field: "`projects`.`id`"},
	Name:      whereHelperstring{field: "`projects`.`name`"},
	LeadID:    whereHelpernull_Uint{field: "`projects`.`lead_id`"},
...
var (
	projectAllColumns            = []string{"id", "name", "lead_id", "created_at", "updated_at"}
	projectColumnsWithoutDefault = []string{"name", "lead_id"}
...
func (o *Project) Lead(mods ...qm.QueryMod) userQuery {
...
func (o *Project) Users(mods ...qm.QueryMod) userQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`project_user` on `users`.`id` = `project_user`.`user_id`"),
...
func (projectL) LoadIssues(ctx context.Context, e boil.ContextExecutor, singular bool, maybeProject interface{}, mods queries.Applicator) error {
	var slice []*Project
	var object *Project
...
// SetLead of the project to the related item.
// Sets o.R.Lead to related.
// Adds o to related.R.LeadProjects.
func (o *Project) SetLead(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &userR{
			LeadProjects: ProjectSlice{o},
		}
	} else {
		related.R.LeadProjects = append(related.R.LeadProjects, o)
	}
...
func (o *Project) RemoveLead(ctx context.Context, exec boil.ContextExecutor, related *User) error {
	var err error
	...
	for i, ri := range related.R.LeadProjects {
		if queries.Equal(o.LeadID, ri.LeadID) {
			continue
		}

		ln := len(related.R.LeadProjects)
		if ln > 1 && i < ln-1 {
			related.R.LeadProjects[i] = related.R.LeadProjects[ln-1]
		}
		related.R.LeadProjects = related.R.LeadProjects[:ln-1]
		break
	}
	return nil
}
...
func (o *Project) AddIssues(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Issue) error {
	var err error
...
// Sets o.R.Project's Issues accordingly.
// Replaces o.R.Issues with related.
// Sets related.R.Project's Issues accordingly.
func (o *Project) SetIssues(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Issue) error {
	query := "update `issues` set `project_id` = null where `project_id` = ?"
...
func (o *Project) RemoveIssues(ctx context.Context, exec boil.ContextExecutor, related ...*Issue) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.ProjectID, nil)
		if rel.R != nil {
...
func (o *Project) AddUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into `project_user` (`project_id`, `user_id`) values (?, ?)"
		values := []interface{}{o.ID, rel.ID}
...
// project replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Projects's Users accordingly.
// Replaces o.R.Users with related.
// Sets related.R.Projects's Users accordingly.
func (o *Project) SetUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "delete from `project_user` where `project_id` = ?"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	removeUsersFromProjectsSlice(o, related)
	if o.R != nil {
		o.R.Users = nil
	}
...
func (o *Project) RemoveUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	var err error
	query := fmt.Sprintf(
		"delete from `project_user` where `project_id` = ? and `user_id` in (%s)",
...
func removeUsersFromProjectsSlice(o *Project, related []*User) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
