...
func testDepartmentToManyUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into `department_user` (`department_id`, `user_id`) values (?, ?)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into `department_user` (`department_id`, `user_id`) values (?, ?)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = a.L.LoadUsers(ctx, tx, false, (*[]*Department)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testDepartmentToManyAddOpUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if first.R.Departments[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}
		if second.R.Departments[0] != &a {
			t.Error("relationship was not added properly to the slice")
		}
...
func testDepartmentToManySetOpUsers(t *testing.T) {
	var err error
	...
	//
	// if len(b.R.Departments) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	// if len(c.R.Departments) != 0 {
	// 	t.Error("relationship was not removed properly from the slice")
	// }
	if d.R.Departments[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
	if e.R.Departments[0] != &a {
		t.Error("relationship was not added properly to the slice")
	}
...
func testDepartmentToManyRemoveOpUsers(t *testing.T) {
	var err error
...
func testDepartmentToOneUserUsingLead(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local Department
	var foreign User

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, departmentDBTypes, true, departmentColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Department struct: %s", err)
	}
	...
	slice := DepartmentSlice{&local}
	if err = local.L.LoadLead(ctx, tx, false, (*[]*Department)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Lead == nil {
...
func testDepartmentToOneSetOpUserUsingLead(t *testing.T) {
	var err error
...
func testDepartmentToOneRemoveOpUserUsingLead(t *testing.T) {
	var err error
...
var (
	departmentDBTypes = map[string]string{`ID`: `int`, `Name`: `varchar`, `LeadID`: `int`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_                 = bytes.MinRead
)
